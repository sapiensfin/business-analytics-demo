...
# 2. ГЕНЕРАЦІЯ ДАНИХ (З РІЗНОЮ СТРУКТУРОЮ МІСЯЦІВ)
@st.cache_data
def get_base_data():
    data = []
    ...
    for month in months:
        m_num = month.month
        # Виручка (нижча в березні та жовтні)
        ...
        # Витрати з РІЗНОЮ динамікою для кожної категорії
        for group, cats in expenses.items():
            for cat, amt in cats.items():
                # Коефіцієнт сезонності для витрат
                if cat == 'Паливо (ПММ)':
                    seasonal_growth = 1.0 + (m_num * 0.05) # Паливо сильно росте до кінця року
                elif cat == 'Зарплата водії':
                    seasonal_growth = 1.0 + (m_num * 0.02) # Зарплати ростуть плавно
                elif cat == 'Ремонт та сервіс' and m_num in [3, 10]:
                    seasonal_growth = 3.0 # Сплески ремонту
                else:
                    seasonal_growth = 1.0 # Постійні витрати стабільні
                
                val = amt * seasonal_growth
...
# 7. КРУГОВІ ДІАГРАМИ (ТЕПЕР З РІЗНОЮ СТРУКТУРОЮ)
st.divider()
...
for i, col in enumerate([c_p1, c_p2]):
    ...
    fig = go.Figure(data=[go.Pie(
        ...
        hole=.4,
        sort=False # щоб зберегти порядок статтей для порівняння
    )])
    fig.update_layout(
        title="Структура у Січні" if i == 0 else "Структура у Грудні", 
        height=400,
...
st.download_button(
    label="📥 Завантажити повний звіт у CSV",
